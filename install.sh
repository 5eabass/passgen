...
# to adapt
# run : passgen init_digest to view your 6 first digit
master_digest = "XXXXXX"
...
if len(sys.argv) < 2 :
	...
	if service == "init_digest" :
		...
		master_digest_init = hashlib.sha512(master_init).hexdigest()[:6]
		...
		# test if the 6 first digit of the digest are corresponding
		if hashlib.sha512(master).hexdigest()[:6] == master_digest : 
			# password to use
			print base64.b64encode(hashlib.sha256(service+master).hexdigest())[30:]
		else :
